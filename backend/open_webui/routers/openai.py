...
async def get_headers_and_cookies(
    ...
    headers = {
        ...
        **(
            {
                'HTTP-Referer': 'https://Lingrai.com/',
                'X-Title': 'Lingrai',
            }
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else 'Lingrai: Server Connection Error',
            )
    ...
    if url_idx is None:
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail='Lingrai: Server Connection Error')
            except Exception as e:
...
def _normalize_stored_item(item: dict) -> dict:
    ...
    Lingrai stores extra bookkeeping fields (``id``, ``status``,
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=r.status if r else 500,
            detail='Lingrai: Server Connection Error',
        )
    finally:
        if not streaming:
            await cleanup_response(r)
